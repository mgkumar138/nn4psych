...
def test_rnn(actor_critic, hp_list, epochs, data_dir, save_dir_behav, save_dir_rnn): 
    ...
    for hp in hp_list:
        name = hp['name']
        values = hp['values']
        file_pattern = hp['file_pattern']
        all_param_states[name] = {'values': values, 'states': []}
        param_cp_lists[name] = []
        param_ob_lists[name] = []
        param_models[name] = []
        param_dicts[name] = {}
        all_param_rnn_activity_cp = []
        all_param_rnn_activity_ob = []
        all_param_rnn_activity = []

        for idx, val in enumerate(values):
            # Format the file pattern with the current value
            file_names = data_dir + file_pattern.format(val)
            models = glob.glob(file_names)
            #models = [f for f in models if float(f.split("\\")[-1].split("_")[0]) > 5]
            print(f"{name}={val}: {len(models)} models found")

            for m, model in enumerate(models):
                all_states, rnn_activity = model_rnn.rnn_predict(model, epochs=epochs)
                all_param_states[name]['states'].append(all_states)
                all_param_rnn_activity.append(rnn_activity)
                
                # rnn_activity is [epochs, 2] (contexts: 0=changepoint, 1=oddball)
                cp_context = [rnn_activity[e, 0] for e in range(rnn_activity.shape[0])]
                ob_context = [rnn_activity[e, 1] for e in range(rnn_activity.shape[0])]
                all_param_rnn_activity_cp.append(cp_context)
                all_param_rnn_activity_ob.append(ob_context)
                
                param_dicts[name][(m, idx)] = {name: val}
                if epochs == 1:
                    param_cp_lists[name].append(all_states[0, 0])
                    param_ob_lists[name].append(all_states[0, 1])
                else:
                    param_cp_lists[name].append(all_states[:, 0])
                    param_ob_lists[name].append(all_states[:, 1])
                param_models[name].append(model)

        # Save behav results
        with open(os.path.join(save_dir_behav, f"{name}_all_param_states.pkl"), "wb") as f:
            pickle.dump(all_param_states[name], f)
        with open(os.path.join(save_dir_behav, f"{name}_dict.pkl"), "wb") as f:
            pickle.dump(param_dicts[name], f)
        with open(os.path.join(save_dir_behav, f"{name}_cp_list.pkl"), "wb") as f:
            pickle.dump(param_cp_lists[name], f)
        with open(os.path.join(save_dir_behav, f"{name}_ob_list.pkl"), "wb") as f:
            pickle.dump(param_ob_lists[name], f)
        
        # Save rnn_activity 
        with open(os.path.join(save_dir_rnn, f"{name}_all_param_rnn_activity_cp.pkl"), "wb") as f:
            pickle.dump(all_param_rnn_activity_cp, f)
        with open(os.path.join(save_dir_rnn, f"{name}_all_param_rnn_activity_ob.pkl"), "wb") as f:
            pickle.dump(all_param_rnn_activity_ob, f)
        # Save rnn_activity for all contexts together
        with open(os.path.join(save_dir_rnn, f"{name}_all_param_rnn_activity.pkl"), "wb") as f:
            pickle.dump(all_param_rnn_activity, f)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--hp_list', type=str, default='all', help='Hyperparameter list')
    parser.add_argument('--epochs', type=int, default=30, help='Number of epochs')
    parser.add_argument('--save_dir', type=str, default='data/rnn_behav/model_params_101000/30_epochs/', help='Directory to save results')

    args = parser.parse_args()

    hp_list = args.hp_list
    epochs = args.epochs
    data_dir = args.data_dir
    save_dir = args.save_dir
    actor_critic = args.model_rnn

    test_rnn(actor_critic, hp_list, epochs, data_dir, save_dir)
